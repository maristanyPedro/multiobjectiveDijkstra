...
inline void merge_3d(TruncatedFront& front, const CostArray& c) {
    if (front.empty()) {
    ...
    while (it != front.end()) {
        if (c[2] <= (*it)[1]) {
            ...
            //Break is only allowed because DIM-1 = 2 and thus we can exploit the advantages of 2d lex. sorting.
            break;
        }
    }
}
